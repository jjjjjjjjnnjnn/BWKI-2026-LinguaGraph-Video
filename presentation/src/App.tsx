...
import { SideRail } from "./components/SideRail"; // v5: right-edge flow indicator
  ...
  return (
    <>
      <Stage onAdvance={stepper.next}>
        <div
          key={`${ch.id}-${stepper.cursor.chapter}-${stepper.cursor.step}`}
          className="scene scene-anim"
        >
          <Cmp step={stepper.cursor.step} />
        </div>
      </Stage>
      {/* v5: SideRail — always-on right-edge flow indicator.
       *  position: fixed but with coords computed via useStageScale so it
       *  follows the actual rendered stage position (which is scaled).
       *  pointer-events: none so the stage-frame underneath still gets
       *  clicks for manual advance. */}
      <SideRail
        step={stepper.cursor.step}
        stepCount={ch.narrations.length}
        totalLabel="PRIVATSCHULE SCHLOSS HEESSEN · BWKI 2026"
      />
      <ProgressBar
        chapters={CHAPTERS}
